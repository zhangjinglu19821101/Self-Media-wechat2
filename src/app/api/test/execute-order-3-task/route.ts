...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[ExecuteOrder3Task] 使用 commandResultId:', commandResultId);

    // 1. 获取所有任务，找到 order_index=3 的任务
    const allTasks = await db
      .select()
      .from(agentSubTasks)
      .where(eq(agentSubTasks.commandResultId, commandResultId));

    const task = allTasks.find(t => t.orderIndex === 3);

    if (!task) {
      return NextResponse.json({ error: '未找到 order_index=3 的任务' }, { status: 404 });
    }

    console.log('[ExecuteOrder3Task] 找到任务:', {
      taskId: task.id,
    ...
    console.log('[ExecuteOrder3Task] 调用 executeAgentTExecutorWorkflow...');
    
    // 使用类型断言访问私有方法
    const engine = subtaskEngine as any;
    await engine.executeAgentTExecutorWorkflow(task);

    console.log('[ExecuteOrder3Task] ========== 任务执行完成 ==========');
    ...
    return NextResponse.json({
      success: true,
      message: 'order_index=3 任务执行完成',
      task: updatedTask
    });

  } catch (error) {
    console.error('[ExecuteOrder3Task] 执行失败:', error);
    return NextResponse.json(
