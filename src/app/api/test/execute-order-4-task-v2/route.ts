...
/**
 * 直接执行 order_index=4 任务的 API 接口
 * 用于测试 Agent T 上传微信公众号草稿箱功能
 */
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[ExecuteOrder4Task] 使用 commandResultId:', commandResultId);

    if (!commandResultId) {
      return NextResponse.json({ error: '未找到可用的 commandResultId，请先创建任务' }, { status: 400 });
    }

    // 1. 获取 order_index=4 的任务
    const allTasks = await db
      .select()
      .from(agentSubTasks)
      .where(eq(agentSubTasks.commandResultId, commandResultId));

    const task = allTasks.find(t => t.orderIndex === 4);

    if (!task) {
      // 尝试列出所有可用的 order_index
      const availableOrders = allTasks.map(t => t.orderIndex);
      return NextResponse.json({ 
        error: '未找到 order_index=4 的任务', 
        availableOrderIndexes: availableOrders,
    ...
    console.log('[ExecuteOrder4Task] ========== 任务执行完成 ==========');
