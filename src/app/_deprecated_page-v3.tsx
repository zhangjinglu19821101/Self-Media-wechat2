...
import { Sparkles, ListTodo, ArrowRight, History, ShieldAlert, Save, CheckCircle2 } from 'lucide-react';
...
function CreationGuideContent() {
  ...
  const handleGenerateFullText = useCallback(async () => {
    ...
    try {
      ...
      // 提取素材 ID
      const materialIds = [
        ...(materialData.keyMaterials || []).map((m: any) => m.id).filter(Boolean),
        ...(materialData.relatedMaterials || []).map((m: any) => m.id).filter(Boolean),
      ];

      // 获取情感基调（从创作控制数据或默认值）
      const emotionTone = creationControlData.emotionTone || 'rational';

      console.log('[HomePage] 🚀 提交创作任务:', {
        userOpinionLength: userOpinion.length,
        materialIdsCount: materialIds.length,
        emotionTone,
        targetWordCount: creationControlData.targetWordCount,
      });

      // 调用 /api/commands 创建任务（与 page.tsx 使用同一 API）
