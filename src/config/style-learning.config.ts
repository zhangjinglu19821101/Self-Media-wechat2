 ...
 * 获取 Agent 的配置
 */
export function getAgentConfig(agent: 'insurance-d' | 'agent-d' | 'insurance-xiaohongshu' | 'insurance-zhihu' | 'insurance-toutiao' | 'deai-optimizer' | 'outline-writer') {
  if (agent === 'insurance-xiaohongshu' || agent === 'insurance-zhihu' || agent === 'insurance-toutiao' || agent === 'deai-optimizer' || agent === 'outline-writer') {
    // 小红书/知乎/头条/去AI化优化/大纲创作 复用 insurance-d 的配置（风格学习通过 style-deposition-service 按模板维度隔离）
 ...
 * 获取 Agent 的文章路径
 */
export function getAgentArticlesPath(agent: 'insurance-d' | 'agent-d' | 'insurance-xiaohongshu' | 'insurance-zhihu' | 'insurance-toutiao' | 'deai-optimizer' | 'outline-writer') {
  if (agent === 'insurance-xiaohongshu' || agent === 'insurance-zhihu' || agent === 'insurance-toutiao' || agent === 'deai-optimizer' || agent === 'outline-writer') {
