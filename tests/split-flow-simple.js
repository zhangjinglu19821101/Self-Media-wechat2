...
const TEST_DAILY_TASK_ID = require('crypto').randomUUID();
...
async function runTests() {
  ...
  try {
    ...
    for (let i = 0; i &lt; subTasks.length; i++) {
      ...
      const newSubTaskId = require('crypto').randomUUID();
    ...
    try {
      ...
      console.log('[WARNING] 清理测试数据失败:', cleanupError);
    }
...
runTests().catch(error =&gt; {
  console.error('[ERROR] 测试运行失败:', error);
  process.exit(1);
});
