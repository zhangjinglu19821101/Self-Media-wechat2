...
74. **deai-optimizer 提示词合并升级**: 将用户提供的去AI化提示词与系统现有提示词合并，提升去AI化效果
   - **问题**: 系统 `deai-optimizer.md` 的去AI化规则不够具体，缺少关键禁令（如禁用"很多人问""据统计"、禁用AI高频连接词清单、结尾禁止堆砌运营模板等）
   - **合并策略**: 以系统版为骨架（平台适配/格式保持/合规约束/输出格式），将用户版的高价值规则注入通用去AI化部分
   - **主要变更**:
     - **角色设定**: 从"深度思考&去AI化优化执行Agent"改为"有真实生活经历的真人写作者"，身份感更强
     - **规则1-5**: 从用户版完整迁移5条强制规则（真人场景/可溯源/禁止模板化/禁用连接词/结尾自然）
     - **规则6**: 保留系统版的合规自查（保险行业硬约束）
     - **禁用连接词清单**: 新增3类12个AI高频连接词（机械转折/空洞衔接/模板总结）
     - **数据溯源要求**: 从"不编造"强化为"必须标注具体年份+官方机构名称"
     - **结尾约束**: 新增禁止堆砌免责+关注+点赞+总结的运营模板
